...
class TranslationNetwork(torch.nn.Module):
    def __init__(self):
        ...
        self.conv1 = torch.nn.Conv2d(in_channels=6, out_channels=32, kernel_size=7, stride=1, padding=3, bias=False)
    ...
    def forward(self, translation_input, wrapped_ref, wrapped_h1, wrapped_h2, T_mels): # Icey: h=w=128
        ...
        # 取出第3个维度为(B,3,1,H,W)的tensor
        one_sketch = target_sketches_split[:, :, 2:3, :, :]
        one_sketch = torch.squeeze(one_sketch, dim=2) # (B, 3, 128, 128)
        ...
        translation_input = torch.cat([gt_mask_face_split, one_sketch], dim=1)
