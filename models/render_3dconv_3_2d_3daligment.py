...
class DenseFlowNetwork(torch.nn.Module): # num_channel=6, num_channel_modulation=3*5
    def __init__(self, num_channel=25, num_channel_modulation=3*5, hidden_size=256):
    ...
    def forward(self, ref_N_frame_img, ref_N_frame_sketch, T_driving_sketch, translation_input): #to output: (B*T,3,H,W)
        ...
        for ref_idx in range(ref_N): # each ref img provide information for each B*T frame # 选择N帧中的1帧
            ...
            flow_module_input = torch.cat((ref_img, ref_sketch, gt_mask_face_split, x_sk), dim=1)  #(B*T, 3+3 +3+16, H, W)
            # Convolutional Layers
