...
class SPADELayer(torch.nn.Module): # input_channel:256 or 64; modulation_channel=3*5; hidden_size=256
    def __init__(self, input_channel, modulation_channel, hidden_size=256, kernel_size=3, stride=1, padding=1,
        ...
        self.position_in = PositionalEmbedding(d_model=d_model,max_len=16384)  #for input image
        self.position_sk = PositionalEmbedding(d_model=d_model,max_len=16384)  #for input sketch
...
# modulation→conv_out-b,256,64,64 -> b,256,4096
        ...
        #print("position_in_encoding",position_in_encoding.size())
        #(2)  modality encoding
...
class DenseFlowNetwork(torch.nn.Module):
    def __init__(self, num_channel=6, num_channel_modulation=3*5, hidden_size=256,
        ...
        #                            T=T,d_model=d_model,nlayers=nlayers,nhead=nhead,dim_feedforward=dim_feedforward,dropout=dropout)
        self.spade_layer_1 = SPADE_ORI(256, num_channel_modulation, hidden_size)
        # self.spade_layer_2 = SPADE(256, num_channel_modulation, hidden_size,
        #                            T=T,d_model=d_model,nlayers=nlayers,nhead=nhead,dim_feedforward=dim_feedforward,dropout=dropout)
        self.spade_layer_2 = SPADE_ORI(256, num_channel_modulation, hidden_size)
        ...
        self.spade_layer_4 = SPADE_ORI(64, num_channel_modulation, hidden_size)
...
class TranslationNetwork(torch.nn.Module):
    def __init__(self, T=1,d_model=256,nlayers=3,nhead=1,dim_feedforward=512,dropout=0.1):
        ...
        # Decoder
        # self.spade_1 = SPADE_ORI(num_channel=256, num_channel_modulation=256)
        self.spade_1 = SPADE(num_channel=256, num_channel_modulation=256,
                             T=T,d_model=d_model,nlayers=nlayers,nhead=2,dim_feedforward=dim_feedforward,dropout=dropout)
        ...
        # self.spade_4 = SPADE(num_channel=64, num_channel_modulation=3,hidden_size=64,
        #                      T=T,d_model=64,nlayers=1,nhead=nhead,dim_feedforward=128,dropout=dropout)
