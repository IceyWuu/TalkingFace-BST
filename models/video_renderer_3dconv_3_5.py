...
class DenseFlowNetwork(torch.nn.Module): # num_channel_modulation=3*5
    def __init__(self, num_channel=6, num_channel_modulation=15, hidden_size=256):
        ...
        self.conv1_3d1 = torch.nn.Conv3d(in_channels=3, out_channels=16, kernel_size=(5,3,3), stride=1, padding=1, bias=False)
        self.conv1_3d1_bn = torch.nn.BatchNorm3d(num_features=16, affine=True)
        self.conv1_3d1_relu = torch.nn.ReLU()

        self.conv1_3d2 = torch.nn.Conv3d(in_channels=3, out_channels=16, kernel_size=(5,3,3), stride=1, padding=1, bias=False)
        ...
        self.conv3_bn = torch.nn.BatchNorm2d(num_features=32, affine=True) # Icey: C from an expected input of size (N,C,H,W), output shape = input
        ...
        self.conv_6= nn.Sequential(torch.nn.Conv2d(47, 32, kernel_size=3, stride=1, padding=1),
                                   torch.nn.ReLU(),
    ...
    def forward(self, ref_N_frame_img, ref_N_frame_sketch, T_driving_sketch): #to output: (B*T,3,H,W)
        ...
        x_for_weight = torch.cat([x_ref_img, x_ref_sk, driving_sketch], dim=1) #(15+16+16)=47
        weight=self.conv_6(x_for_weight) #  (B,1,128,128)
        # 拼起来
        x_cat = torch.cat([x_ref_img, x_ref_sk], dim=1) # (B,32,128,128)
        h1 = self.conv3_relu(self.conv3_bn(self.conv3(x_cat)))*weight # (B,32,128,128)
        h2 = self.conv2_relu(self.conv2_bn(self.conv2(h1)))*downsample(weight, (64,64))    #(256,64,64)
