...
class Fusion_transformer(nn.Module): # d_model是embedding size，字符转换为词向量的维度；dim_feedforward是前馈神经网络中线性层映射到的维度
    def __init__(self,T, d_model, nlayers, nhead, dim_feedforward,  # 1024   128 #Icey 输入的embdding维度d_model=512, 经过feedforward后的维度dim_feedforward=1024
        ...
        # 长度为4的张量，每个大小是512。torch.nn包下的Embedding，作为训练的一层，随模型训练得到适合的词向量
    ...
    def forward(self,h2_embedding, sketch_embedding):
                #[h2(B*T,256,64,64), driving_sketch(B*T, 3, H, W)]-> (B*T, xxxx)!!!!!!
        # (1).  positional(temporal) encoding
        position_h2_encoding = self.position_h2(h2_embedding) # (B,T,512)
        position_sketch_encoding = self.position_sk(sketch_embedding) # (B,T,512)
        ...
        modality_h2 = self.modality(1 * torch.ones((h2_embedding.size(0), self.T), dtype=torch.int).cuda())
        modality_sk = self.modality(2 * torch.ones((sketch_embedding.size(0),  self.T), dtype=torch.int).cuda())
        ...
        h2_tokens = h2_embedding + position_h2_encoding + modality_h2 #(B , T, 512 )
        sketch_tokens = sketch_embedding + position_sketch_encoding + modality_sk #(B , T, 512 )
        ...
        encoder_output = self.transformer_encoder(h2_tokens)
        decoder_output = self.transformer_decoder(sketch_tokens, encoder_output) # sketch作为q，h2作为kv
...
class DenseFlowNetwork(torch.nn.Module):
    def __init__(self, num_channel=6, num_channel_modulation=3*5, hidden_size=256):
        ...
        # driving_sketch (B*T, 3, 128, 128)
        self.driving_sketch_flatten = nn.Sequential(
            Conv2d(3, 32, kernel_size=3, stride=1, padding=1), # (32,128,128)
        ...
        # T=1 d_model=? nlayers=3 nhead=? dim_feedforward=? dropout=0.1 !!!!!
        self.fusion_transformer = Fusion_transformer(T=1,d_model=512,nlayers=3,nhead=4,dim_feedforward=256,dropout=0.1)
    ...
    def forward(self, ref_N_frame_img, ref_N_frame_sketch, T_driving_sketch): #to output: (B*T,3,H,W)
        ...
        for ref_idx in range(ref_N): # each ref img provide information for each B*T frame # 选择N帧中的1帧
            ...
            # Transformer↓--------------------------------------------------------
            # !!!!!!在这用encoder、decoder 两个输入？
            
            # !!!!!! #[h2(B*T,256,64,64), driving_sketch(B*T, 3, H, W)]-> (B*T, xxxx) !!!!!!
            h1 = self.h2_flatten(h2)
            driving_sketch = self.driving_sketch_flatten(driving_sketch)

            #normalization
            h2 = self.Norm(h2)
            driving_sketch = self.Norm(driving_sketch)
            transformer_layer = self.fusion_transformer(h2, driving_sketch)
            transformer_layer = self.pixel_shuffle_1(transformer_layer)
...
class Renderer(torch.nn.Module):
    # d_model, dim_feedforward 不确定是什么，先按照landmark_generator的!!!!!!
    def __init__(self, T=1, d_model=512, nlayers=3, nhead=4, dim_feedforward=1024, dropout = 0.1): # transformer的encoder和decoder（3层）
        super(Renderer, self).__init__()

        # 1.flow Network
        # self.flow_module = DenseFlowNetwork()
        self.flow_module = DenseFlowNetwork(T,d_model,nlayers,nhead,dim_feedforward,dropout)
