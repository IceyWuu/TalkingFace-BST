...
from models.video_renderer_3dconv_3_2d_flow import Renderer 
...
#other parameters
num_workers = 20
# Project_name = 'conv3_3_2d_epo92_encoder'   #Project_name
...
def save_sample_images_gen(T_mels_lmk,T_frame_sketch,T_frame_sketch_from_pre, ref_N_frame_img,ref_N_frame_sketch, generated_img, gt, checkpoint_dir):
    ...
    ref_N_frame_sketch = ref_N_frame_sketch.unsqueeze(1).expand(-1, T_each_batch, -1, -1, -1, -1)  # (B,T,ref_N,3,H,W)
    ref_N_frame_sketch = (ref_N_frame_sketch.cpu().numpy().transpose(0, 1, 2, 4, 5, 3) * 255.).astype(np.uint8)  # ref: (B,T,ref_N,H,W,3)
    ...
    collage_lm = np.concatenate(( *[T_frame_sketch[:, :, i] for i in range(T_frame_sketch.shape[2])],
                            *[T_frame_sketch_from_pre[:, :, i] for i in range(T_frame_sketch_from_pre.shape[2])]),
                             axis=-2)
    for batch_idx, c in enumerate(collage_lm):   # require (B,T,H,W,3)
        for t in range(len(c)):
