...
# parser.add_argument('--input', '--input_template_video', type=str, default='/data/wuyubing/TalkingFace-BST/test/template_video/129.mp4')
parser.add_argument('--input', '--input_template_video', type=str, default='/data/wuyubing/TalkingFace-BST/test/test_video/me_sk_random/me_noaudio.mp4')
#'./test/template_video/129.mp4'

# parser.add_argument('--audio', type=str, default='/data/wuyubing/TalkingFace-BST/test/template_video/audio2.wav')
parser.add_argument('--audio', type=str, default='/data/wuyubing/TalkingFace-BST/test/test_video/me_sk_random/00011.mp3')
#'./test/template_video/abstract.mp3'
#'./test/template_video/audio2.wav'

parser.add_argument('--output_dir', type=str, default='/data/wuyubing/TalkingFace-BST/test_result')
...
video_save_path = 'conv3_B64_3_aligment'
# ori_B64_may
# conv3_1
# conv3_B64_2
# conv3_B64_3_2d
# conv3_B64_1_onesk
# conv3_B64_3_aligment

device = 'cuda' if torch.cuda.is_available() else 'cpu'
args = parser.parse_args()

folder_full_gen = '/data/wuyubing/TalkingFace-BST/test/test_video/full_gen/{}/{}'.format(args.input.split('/')[-2], args.renderer_checkpoint_path.split('/')[-2])
# folder_ori = '/data/wuyubing/TalkingFace-BST/test/test_video/ori/{}/{}'.format(args.input.split('/')[-2], args.renderer_checkpoint_path.split('/')[-2])
folder_full_ori = '/data/wuyubing/TalkingFace-BST/test/test_video/full_ori/{}/{}'.format(args.input.split('/')[-2], args.renderer_checkpoint_path.split('/')[-2])

os.makedirs(folder_full_gen, exist_ok=True)
# os.makedirs(folder_ori, exist_ok=True)
os.makedirs(folder_full_ori, exist_ok=True)
...
temp_dir = 'tempfile_of_{}/{}'.format(output_dir.split('/')[-1],args.renderer_checkpoint_path.split('/')[-2])
...
outfile_path = os.path.join(output_dir,
                       '{}_N_{}_Nl_{}.mp4'.format(video_save_path, ref_img_N, Nl))
if os.path.isfile(input_video_path) and input_video_path.split('.')[1] in ['jpg', 'png', 'jpeg']:
    args.static = True
...
with mp_face_mesh.FaceMesh(static_image_mode=True, max_num_faces=1, refine_landmarks=True,
    ...
    for frame_idx, full_frame in enumerate(ori_background_frames):
        h, w = full_frame.shape[0], full_frame.shape[1]
        # print("full_frame",type(full_frame)) #  <class 'numpy.ndarray'>
...
for batch_idx, batch_start_idx in tqdm(enumerate(range(0, input_mel_chunks_len - 2, 1)), # 1 个chunk是5帧，16个mel
    ...
    #print("full",full.shape)#(160,160,3)
    # 6.output
    full = np.concatenate([show_sketch, full], axis=1) # ([show_sketch, full], axis=1)

    
    cv2.imwrite('{}/{}.png'.format(folder_full_gen, batch_idx), full)
    # ori_face_img = (ori_face_img*255).squeeze(0).squeeze(0).permute(1, 2, 0)
    # cv2.imwrite('{}/{}_{}.png'.format(folder_ori, batch_idx), ori_face_img)
    cv2.imwrite('{}/{}.png'.format(folder_full_ori, batch_idx), original_background)
